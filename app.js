const app = {
    ...
    finishOnboarding() {
        ...
        const money = this.getRawNumber(document.getElementById('ob-money').value);
        const budget = this.getRawNumber(document.getElementById('ob-budget').value);

        if (budget < 100000) {
            this.showToast('Budget minimal Rp100.000 ya!');
            return;
        }
    ...
    renderQuickLogEdit() {
        ...
        this.state.quickLogs.forEach(ql => {
            list.innerHTML += `
                <div class="ql-edit-item">
                    <div>
                        <div class="font-semibold text-sm">${ql.name}</div>
                        <div class="text-xs text-muted">${this.formatRp(ql.amount)} • ${ql.category}</div>
                    </div>
                    <button class="icon-btn" onclick="app.removeQuickLog(${ql.id})"><i class="ph ph-trash"></i></button>
                </div>
            `;
        });
    },
    ...
    updateHomeView() {
        ...
        if (spentDay === 0) {
            ...
            statusEl.innerHTML = '<i class="ph-fill ph-check-circle"></i><span id="home-daily-msg">Belum ada pengeluaran. Nice.</span>';
        } else if (spentDay > dailyBudget * 1.5) {
            statusEl.className = 'daily-status warning';
            statusEl.innerHTML = '<i class="ph-fill ph-warning-circle"></i><span id="home-daily-msg">Waduh, agak kalap.</span>';
        } else if (spentDay > dailyBudget) {
            statusEl.className = 'daily-status warning';
            statusEl.innerHTML = '<i class="ph-fill ph-info"></i><span id="home-daily-msg">Sedikit lewat budget harian.</span>';
        } else {
            statusEl.className = 'daily-status';
            statusEl.innerHTML = '<i class="ph-fill ph-check-circle"></i><span id="home-daily-msg">Aman, masih on track.</span>';
        }
        ...
        txList.innerHTML = '';
        if (dayTxs.length === 0) {
            txList.innerHTML = `<div class="tx-empty">Belum ada pengeluaran<br>Semoga memang hemat.</div>`;
        } else {
            dayTxs.forEach(t => {
    ...
    saveManualTransaction() {
        ...
        if (amtInput <= 0) {
            this.showToast('Nominal belum diisi atau tidak valid');
            return;
        }
    ...
    updateProfilView(searchTerm = '') {
        ...
        if (Object.keys(grouped).length === 0) {
            list.innerHTML = `<div class="tx-empty">Belum ada transaksi di sini.</div>`;
            return;
        }
        ...
        for (const [dateStr, txs] of Object.entries(grouped)) {
            ...
            groupEl.innerHTML = `
                <div class="history-date-header" onclick="app.toggleHistory('${headerId}')">
                    <span>${headerDate} • ${txs.length} transaksi</span>
                    <div style="display:flex; align-items:center; gap:8px;">
                        <span class="text-main font-bold">${this.formatRp(totalDay)}</span>
    ...
    exportExcel() {
        ...
        let table = `<table border="1">
            <tr>
                <th style="background-color: #00B873; color: white;">Tanggal</th>
                <th style="background-color: #00B873; color: white;">Waktu</th>
                <th style="background-color: #00B873; color: white;">Nama Pengeluaran</th>
                <th style="background-color: #00B873; color: white;">Kategori</th>
                <th style="background-color: #00B873; color: white;">Jumlah (Rp)</th>
                <th style="background-color: #00B873; color: white;">Catatan</th>
            </tr>`;
